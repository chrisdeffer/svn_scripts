...
1;
(netcool@eurzp3503)/users/netcool# cat /opt/netcool/omnibus/probes/wfotherscripts/svn_script_modules/Netcool.pm
package Netcool;
use strict;
use warnings;
use Log::Log4perl;
...
sub new
{
    ...
    my $log_conf = shift || "$OMNIHOME/probes/wfotherscripts/log.conf";
...
sub pastart {
    ...
    if ( $command =~ /Error/ )
    {
    $l->info("Error when running nco_pa_start");
        $result = 0;
    }
    else
    {
...
sub pastop {
    ...
    if ( $command =~ /Error/ )
    {
    $l->info("Error when running nco_pa_stop");
        $result = 0;
    }
    else
    {
